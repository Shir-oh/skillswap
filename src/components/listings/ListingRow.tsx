...
export default function ListingRow({ locale, listing }: Props) {
  ...
  return (
    <Link
      ...
      className="flex flex-col items-center gap-4 rounded-2xl p-4 outline outline-white/10 transition hover:bg-white/10 sm:flex-row sm:items-center"
    >
      <Image
        src={listing.image}
        alt={listing.name}
        width={400}
        height={240}
        className="rounded-xl object-cover sm:h-24 sm:w-24 sm:shrink-0 lg:h-32 lg:w-60"
      />
